...
def generate_readme_content(folder_name, checks_data):
    ...
    content += "### ⚡ Дополнительные возможности CLI\n\n"
    content += "**Сокращенные флаги:** Вы можете использовать как `--from-file`, так и `-f`:\n"
    content += "```bash\n"
    content += "pngr checks create -f папка/check.json\n"
    content += "pngr checks run custom -f папка/check.json\n"
    content += "```\n\n"
    content += "**Удаленные файлы:** CLI поддерживает загрузку файлов по URL:\n"
    content += "```bash\n"
    content += "pngr checks create -f https://raw.githubusercontent.com/pingera/marketplace/refs/heads/main/multistep/simple.json\n"
    content += "pngr checks run custom -f https://raw.githubusercontent.com/pingera/marketplace/refs/heads/main/synthetic/simple-screenshot.json\n"
    content += "```\n\n"
